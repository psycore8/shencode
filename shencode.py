...
def main(command_line=None):
  ...
  # modules: create_msf, create_xstub 
  ...
  if arguments.command == 'msfvenom':
  ...
  elif arguments.command == 'xorstub':
    stub.xor.Input_File = arguments.input
    stub.xor.XOR_Key = arguments.key
    stub.xor.Output_File = arguments.output
    ...
    stub.xor.process()
      ...
      print(scFormat)
      if arguments.write:
        assist.FileManipulation.WriteToTemplate(arguments.write, scFormat)
      ...
      if (ror_key < 32) or (ror_key > 255):
        ...
        exit()
      ror2rol = sc.ror2rol
      ror2rol.process(dll_paths, arguments.input, arguments.output, arguments.key)
      ...
      try: 
        with open(arguments.input, "rb") as file:
          ...
          print(f"{nstate.FAIL} File not found or cannot be opened.")
          exit()
      modified_shellcode = xor.xor_crypt_bytes(shellcode, int(arguments.key))
